...
public class DrawsanaView: UIView {
    ...
    private var dragHandler: ResizeAndRotateShapeHandler?
    ...
    public let deleteSelectedView = UIView(frame:CGRect(x: -33, y: -33, width: 30, height: 30))
    public let resizeSelectedView = UIView(frame:CGRect(x: -30, y: -30, width: 30, height: 30))
    ...
    private func commonInit() {
        ...
        selectionIndicatorView.addSubview(deleteSelectedView)
        
        
        selectionIndicatorView.addSubview(resizeSelectedView)
    ...
    private func _didPan(sender: ImmediatePanGestureRecognizer) {
        ...
        if let selectedShape = toolSettings.selectedShape, deleteSelectedView.convert(deleteSelectedView.bounds, to: self).contains(point) {
        ...
        case .began:
            ...
            if let selectedShape = toolSettings.selectedShape, resizeSelectedView.convert(resizeSelectedView.bounds, to: self).contains(point) {
                dragHandler = ResizeAndRotateShapeHandler(shape: selectedShape)
                dragHandler?.handleDragStart(context: toolOperationContext, point: point)
                isResizing = true
            } else {
        ...
        case .changed:
            if isResizing {
                dragHandler?.handleDragContinue(context: toolOperationContext, point: point, velocity: sender.velocity ?? .zero)
        ...
        case .ended:
            if isResizing {
                dragHandler?.handleDragEnd(context: toolOperationContext, point: point)
            } else {
    ...
    private func applySelectionViewState() {
        ...
        resizeSelectedView.frame = CGRect(x: selectionIndicatorView.bounds.maxX, y: selectionIndicatorView.bounds.maxY, width: 30, height: 30)
        
        if toolSettings.selectedShape as? TextTool != nil {
            resizeSelectedView.isHidden = true
            deleteSelectedView.isHidden = true
        } else {
            resizeSelectedView.isHidden = false
            deleteSelectedView.isHidden = false
        }
    }
